...
A child process cannot generally set environment variables in its parent shell. For shell scripts, use [JSON or dotenv output][json-or-dotenv-output] and explicitly pass the result to later commands.

For .NET builds, see the [MSBuild integration][msbuild-integration]. For every available value and its meaning, see [version variables][version-variables].

[ci-guides]: /docs/reference/build-servers

[v6-to-v7-migration-guide]: /docs/migration/v6-to-v7#environment-variables

[json-or-dotenv-output]: /docs/usage/cli/output

[msbuild-integration]: /docs/usage/msbuild
