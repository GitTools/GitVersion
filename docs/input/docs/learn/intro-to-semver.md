...
For the official Semantic Version docs head to [semver.org][semver-org]. This is just a quick guide for people  getting started and how SemVer is used in GitVersion.
...
For a more complete explanation check out [semver.org][semver-org] which is the official spec. Remember this is a brief introduction and does not cover all parts of semantic versioning, just the important parts to get started.

## SemVer in GitVersion

GitVersion makes it easy to follow semantic versioning in your library by automatically calculating the next semantic version which your library/application is likely to use. In [GitFlow][gitflow] the develop branch will bump the _minor_ when main is tagged, while [GitHubFlow][githubflow] will bump the _patch_.

Because one size does not always fit all, GitVersion provides many [Variables][variables] for you to use which contain different variations of the version. For example SemVer will be in the format `{major}.{minor}.{patch}-{tag}`, but `FullSemVer` will also include build metadata: `{major}.{minor}.{patch}-{tag}+{buildmetadata}`

[semver-org]: https://semver.org

[gitflow]: https://www.atlassian.com/git/tutorials/comparing-workflows/gitflow-workflow/

[githubflow]: https://docs.github.com/en/get-started/quickstart/github-flow#introduction
