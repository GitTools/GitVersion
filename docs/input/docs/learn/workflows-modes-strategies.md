...
A workflow loads a built-in configuration. Your configuration can override it. [Choose a workflow][choose-a-workflow] and inspect the result with `dotnet-gitversion --show-config`.

## Deployment modes shape the result

The effective branch configuration determines the deployment mode. Compare the [deployment modes][deployment-modes] and their examples before selecting one. The same commit count does not imply the same pre-release version under every mode.
...
See the [strategy reference][strategy-reference] for each strategy's role, then read [how calculation works][how-calculation-works].

## Keep output separate

After calculation, GitVersion exposes [variables][variables]. CLI output options choose how to consume them; assembly and custom formatting settings produce additional version representations. Changing the output destination does not select a new workflow.

[choose-a-workflow]: /docs/usage/choose-workflow

[deployment-modes]: /docs/reference/modes

[strategy-reference]: /docs/reference/version-sources

[how-calculation-works]: /docs/learn/how-it-works
