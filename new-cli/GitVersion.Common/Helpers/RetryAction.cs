...
public class RetryAction<T, TResult> where T : Exception
{
    private readonly RetryPolicy<TResult> retryPolicy;
    ...
    public RetryAction(int maxRetries = 5)
    {
        ...
        this.retryPolicy = Policy<TResult>
            .Handle<T>()
            .WaitAndRetry(linearBackoff);
    }

    public TResult Execute(Func<TResult> operation) => this.retryPolicy.Execute(operation);
