...
namespace GitVersion
{
    internal sealed class GitRepository : IMutatingGitRepository
    {
        ...
        public void Checkout(string commitOrBranchSpec) =>
            RepositoryExtensions.RunSafe(() => Commands.Checkout(repositoryInstance, commitOrBranchSpec));

        public void Fetch(string remote, IEnumerable<string> refSpecs, AuthenticationInfo auth, string logMessage) =>
            RepositoryExtensions.RunSafe(() => Commands.Fetch((Repository)repositoryInstance, remote, refSpecs, GetFetchOptions(auth), logMessage));
        ...
        private static CloneOptions GetCloneOptions(AuthenticationInfo auth) => new()
        {
            Checkout = false,
            CredentialsProvider = GetCredentialsProvider(auth)
        };
