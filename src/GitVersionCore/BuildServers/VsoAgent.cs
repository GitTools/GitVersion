namespace GitVersion
{
	...
	public class VsoAgent : BuildServerBase
	{
		...
		{
			if (Environment.GetEnvironmentVariable("BUILD_SOURCEBRANCH") != null)
				return Environment.GetEnvironmentVariable("BUILD_SOURCEBRANCH");

			//TFS2013 Support.  Must parse branch from TF_BUILD_SOURCEGETVERSION.
			if (Environment.GetEnvironmentVariable("TF_BUILD_SOURCEGETVERSION") != null)
				return Regex.Replace(Environment.GetEnvironmentVariable("TF_BUILD_SOURCEGETVERSION") ?? "", @".*(refs\/heads\/.*?):.*", "$1");
		...
		public override string GenerateSetVersionMessage(VersionVariables variables)
		{
			...
			// specified
			var buildNum = Environment.GetEnvironmentVariable("BUILD_BUILDNUMBER") ?? Environment.GetEnvironmentVariable("TF_BUILD_BUILDNUMBER");
