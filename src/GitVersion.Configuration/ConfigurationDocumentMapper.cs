...
internal static class ConfigurationDocumentMapper
{
    ...
    public static ConfigurationDocumentKind Detect(IReadOnlyDictionary<object, object?> document)
    {
        ...
        if (document.Count == 1 && document.ContainsKey(WorkflowPropertyName))
        {
            return ConfigurationDocumentKind.Shared;
        }
        ...
        var hasFlat = document.Keys.OfType<string>().Any(key =>
            ...
            && !key.Equals(WorkflowPropertyName, StringComparison.Ordinal));
        ...
        string source)
    {
        ...
        if (kind == ConfigurationDocumentKind.Mixed)
        {
            throw new ConfigurationException(
                $"The {source} combines v7 'calculation'/'output' sections with unsupported root-level properties. " +
                "Only 'workflow' is allowed alongside these sections. Check for misplaced or misspelled properties; " +
                "for a v6 configuration, run 'gitversion config migrate'.");
        }
    ...
    public static Dictionary<object, object?> NormalizeInternal(IReadOnlyDictionary<object, object?> document, string source)
    {
        ...
        return kind switch
        {
            ...
            ConfigurationDocumentKind.Shared or ConfigurationDocumentKind.V6 => CloneDictionary(document),
            ConfigurationDocumentKind.V7 => Flatten(document),
            _ => throw new ConfigurationException(
                $"The {source} combines v7 'calculation'/'output' sections with unsupported root-level properties. Only 'workflow' is allowed alongside these sections.")
        };
    }
    ...
    public static Dictionary<object, object?> Flatten(IReadOnlyDictionary<object, object?> document)
    {
        ...
        if (document.TryGetValue(WorkflowPropertyName, out var workflow))
        {
            result[WorkflowPropertyName] = CloneValue(workflow);
        }
    ...
    public static Dictionary<string, object?> Nest(IReadOnlyDictionary<string, object?> document)
    {
        ...
        {
            if (key.Equals(WorkflowPropertyName, StringComparison.Ordinal))
            {
                result[key] = value;
            }
            else if (key.Equals(BranchesPropertyName, StringComparison.Ordinal))
            {
        ...
        result[CalculationSectionName] = calculation;
        result[OutputSectionName] = output;
        return result;
    }
    ...
    public static Dictionary<object, object?> Nest(IReadOnlyDictionary<object, object?> document)
    {
        ...
        foreach (var (key, value) in document)
        {
            if (key is string rootPropertyName && rootPropertyName.Equals(WorkflowPropertyName, StringComparison.Ordinal))
            {
                result[key] = CloneValue(value);
            }
            else if (key is string propertyName && propertyName.Equals(BranchesPropertyName, StringComparison.Ordinal))
            {
                SplitBranches(value, calculation, output);
            }
    ...
    {
        if (propertyName.Equals(WorkflowPropertyName, StringComparison.Ordinal))
        {
            throw new ConfigurationException($"Configuration property '{sectionName}.workflow' belongs at the root as 'workflow'.");
        }
