...
internal class OverrideConfigurationOptionParser
{
    ...
    {
        if (segments.Count == 1 && segments[0] == ConfigurationDocumentMapper.WorkflowPropertyName)
        {
            return true;
        }
        ...
        {
            return segments[1] != ConfigurationDocumentMapper.WorkflowPropertyName
    ...
    {
        if (IsNestedRootPath(segments) && segments[1] == ConfigurationDocumentMapper.WorkflowPropertyName)
        {
            return ConfigurationDocumentMapper.WorkflowPropertyName;
        }
    ...
    private static void NormalizeSegments(IList<string> segments)
    {
        var branchesIndex = -1;
        if (segments.Count >= 3
            && segments[0].Equals(ConfigurationDocumentMapper.BranchesPropertyName, StringComparison.OrdinalIgnoreCase))
        {
            branchesIndex = 0;
        }
        else if (segments.Count >= 4
                 && segments[1].Equals(ConfigurationDocumentMapper.BranchesPropertyName, StringComparison.OrdinalIgnoreCase))
        {
            branchesIndex = 1;
        }
        ...
        {
            var isBranchNameSegment = branchesIndex >= 0
                                      && index > branchesIndex
                                      && index < segments.Count - 1;
            if (!isBranchNameSegment)
            {
    ...
    private static bool IsAmbiguousBranchPath(IReadOnlyList<string> segments, out string? branchName)
    {
        var branchesIndex = -1;
        if (segments.Count > 3 && segments[0] == ConfigurationDocumentMapper.BranchesPropertyName)
        {
            branchesIndex = 0;
        }
        else if (segments.Count > 4
        ...
        var isAmbiguous = branchesIndex >= 0
                          && _lazySupportedBranchProperties.Value.Contains(segments[^1]);

        branchName = isAmbiguous
            ? string.Join('.', segments.Skip(branchesIndex + 1).SkipLast(1))
            : null;
        return isAmbiguous;
    }
