    ...
    : IArgumentParser
{
        ...
        bool firstArgumentIsSwitch)
    {
        ...
        foreach (var key in switchesAndValues.AllKeys)
        {
            ...
            // The switch names handled by the two helpers are mutually exclusive,
            // so a given key is matched by at most one of them.
            AddCommonSwitchTelemetry(telemetry, key, values, value);
            AddRemainingSwitchTelemetry(telemetry, key, value);
        }
    }

    private static void AddCommonSwitchTelemetry(TelemetryCollectionBuilder telemetry, string key, string[]? values, string? value)
    {
        if (key.IsSwitch("l"))
        {
    ...
    private static void AddRemainingSwitchTelemetry(TelemetryCollectionBuilder telemetry, string key, string? value)
    {
        if (key.IsSwitch("nonormalize"))
        {
    ...
    private bool ParseSwitches(Arguments arguments, string? name, IReadOnlyList<string>? values, string? value)
    {
        ...
        this.loggingLevelSwitch.MinimumLevel = VerbosityMaps[verbosity];
        return true;
    }
