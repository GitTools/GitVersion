    ...
    : IGitPreparer
{
    ...
    private void ChooseLocalBranchToAttach(string? headSha, IReadOnlyCollection<IBranch> localBranches)
    {
        ...
        this.logger.LogWarning($"Found more than one local branch pointing at the commit '{headSha}' ({csvNames}).");
        ...
        else
        {
            ...
            if (branchesWithoutSeparators.Count == 1)
            {
                ...
                this.logger.LogWarning($"Choosing {branchWithoutSeparator.Name.Canonical} as it is the only branch without / or - in it. " + moveBranchMsg);
