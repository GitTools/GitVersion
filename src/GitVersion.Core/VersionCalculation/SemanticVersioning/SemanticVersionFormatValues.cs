...
namespace GitVersion
{
    public class SemanticVersionFormatValues
    {
        ...
        public string NuGetPreReleaseTagV2 => this.semver.PreReleaseTag.HasTag() ? this.semver.PreReleaseTag.ToString("lp").ToLower() : null;
        ...
        private string GetWeightedPreReleaseNumber()
        {
            var weightedPreReleaseNumber =
                this.semver.PreReleaseTag.HasTag() ? (this.semver.PreReleaseTag.Number + this.config.PreReleaseWeight).ToString() : null;
            if (string.IsNullOrEmpty(weightedPreReleaseNumber))
            {
                weightedPreReleaseNumber = $"{this.config.TagPreReleaseWeight}";
            }
