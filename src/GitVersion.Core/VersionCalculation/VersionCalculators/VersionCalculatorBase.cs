    ...
    ILogger logger, IRepositoryStore repositoryStore, Lazy<GitVersionContext> versionContext)
{
    ...
    protected SemanticVersionBuildMetaData CreateVersionBuildMetaData(ICommit? baseVersionSource)
    {
        ...
        this.logger.LogInformation("{CommitsSinceTag} commits found between {BaseVersionSource} and {CurrentCommit}", commitsSinceTag, baseVersionSource, Context.CurrentCommit);
